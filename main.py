...
def submit():
    if request.method=='POST':
        if save_new_file(myfile):
            hfile = request.files['myfile']
        #image_path = "/sample" + myfile.filename
        #myfile.save(image_path)
        #display_img=Image.open(myfile)
        #st.image(display_img)
            features=feature_extraction(os.path.join('inputs',hfile.name),model)
        #st.text(features)
            indices=recommend(features,feature_list)
        #c1,c2,c3,c4,c5 = st.columns(5)
        #with c1:
            temp=cv2.imread(file_names[indices[0][0]])
    return render_template('result.html')


@main.route('/result')
def result():
    return cv2.imshow('output', cv2.resize(temp, (512, 512)))
    return render_template('result.html')
